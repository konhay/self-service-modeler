...
describing<-function(qstr, vtype, count){
	...
	if(class(msg)[1]=="SparkDataFrame" && length(colnames(msg)!=0)){
		...
		if(ncolumn< 2 | ncolumn > (max_col+1)){# including label column
			msg<-paste("变量个数太多或不足(", ncolumn-1, "/", max_col, ")", sep="")
			cat("[EXCEPTION]", msg, "\n", file=filename, append=T)
			sparkR.session.stop()
			return(c(1, msg))
		}
		...
		if(ct < min_count | ct > max_count){
			msg<-paste("输入的数据集规模(", ct, ")小于下限(", min_count, ")或大于上限(", max_count, ")", sep="")
			cat("[EXCEPTION]", msg, "\n", file=filename, append=T)
			sparkR.session.stop()
			return(c(1, msg))
		}
...
modeling<-function(teller, qstr, model){
	...
	if(class(msg)[1]=="SparkDataFrame" && length(colnames(msg)!=0)){
		...
		if(larger/smaller > label_ratio){
			msg<-paste("样本正负比例悬殊，超过预设阀值(", label_ratio, "/", round(larger/smaller, 1), ")", sep="")
			cat("[EXCEPTION]", msg, "\n", file=filename, append=T)
			sparkR.session.stop()
			return(c(1, msg))
		}
		...
		if(model_name=="DT"){
			...
			msg<-paste("无法识别的模型类型(", model_name, ")", sep="")
			cat("[EXCEPTION]", msg, "\n", file=filename, append=T)
			sparkR.session.stop()
			return(c(1, msg))
		}
